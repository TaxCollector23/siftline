...
type Bench = {
  ...
  officialPricing: { sourceUrl: string; verifiedOn: string; scope: string; models: OfficialApiComparison[] };
};
...
const fallback: Bench = {
  ...
  officialPricing: {
    ...
    models: [
      { model: "gpt-6-astra", label: "GPT-6 Astra", inputPerMillion: 10, cachedInputPerMillion: 1, outputPerMillion: 50, baselineInputCostUsd: 53.83, cutdexInputCostUsd: 9.84, savedInputCostUsd: 43.99, inputCostReductionPercent: 81.7 },
      { model: "gpt-6-sol", label: "GPT-6 Sol", inputPerMillion: 2, cachedInputPerMillion: 0.2, outputPerMillion: 10, baselineInputCostUsd: 10.77, cutdexInputCostUsd: 1.97, savedInputCostUsd: 8.80, inputCostReductionPercent: 81.7 },
      { model: "gpt-6-luna", label: "GPT-6 Luna", inputPerMillion: 0.1, cachedInputPerMillion: 0.01, outputPerMillion: 0.5, baselineInputCostUsd: 0.54, cutdexInputCostUsd: 0.10, savedInputCostUsd: 0.44, inputCostReductionPercent: 81.7 },
      { model: "gpt-5.6-sol", label: "GPT-5.6 Sol", inputPerMillion: 4, cachedInputPerMillion: 0.4, outputPerMillion: 20, baselineInputCostUsd: 21.53, cutdexInputCostUsd: 3.94, savedInputCostUsd: 17.60, inputCostReductionPercent: 81.7 },
      { model: "gpt-5.3-codex", label: "GPT-5.3 Codex", inputPerMillion: 3.5, cachedInputPerMillion: 0.35, outputPerMillion: 28, baselineInputCostUsd: 18.84, cutdexInputCostUsd: 3.44, savedInputCostUsd: 15.40, inputCostReductionPercent: 81.7 },
    ],
  },
};
...
function ModelPricingTable({ bench }: { bench: Bench }) {
  return <section className="model-pricing wrap" id="model-pricing"><div className="model-pricing-head"><div><p className="eyebrow">Official API sensitivity</p><h2>Same measured reduction. Different model rates.</h2></div><p>These are input-only scenarios at the standard short-context API rates published by OpenAI. The token reduction is measured here; the dollar column is a rate-sensitive calculation, not a provider invoice.</p></div><div className="comparison-scroll"><table className="comparison-table pricing-table"><thead><tr><th scope="col">Model</th><th scope="col">Input / 1M</th><th scope="col">Before Cutdex</th><th scope="col">With Cutdex</th><th scope="col">Modeled input saved</th></tr></thead><tbody>{bench.officialPricing.models.map((model) => <tr key={model.model}><th scope="row">{model.label}</th><td>${model.inputPerMillion.toLocaleString()}</td><td>${model.baselineInputCostUsd.toFixed(2)}</td><td>${model.cutdexInputCostUsd.toFixed(2)}</td><td className="pricing-saved">${model.savedInputCostUsd.toFixed(2)} <small>({model.inputCostReductionPercent.toFixed(1)}%)</small></td></tr>)}</tbody></table></div><p className="comparison-note">Verified {bench.officialPricing.verifiedOn}. <a href={bench.officialPricing.sourceUrl} rel="noreferrer">Open the official OpenAI pricing page ↗</a> · {bench.officialPricing.scope}</p></section>;
}
