...
type Bench = {
  ...
  officialPricing: { sourceUrl: string; subscriptionSourceUrl: string; verifiedOn: string; scope: string; models: OfficialApiComparison[] };
};

type OfficialApiComparison = {
  model: string;
  label: string;
  inputPerMillion: number;
  cachedInputPerMillion: number;
  outputPerMillion: number;
  planInputCreditsPerMillion: number | null;
  plusMessagesPerFiveHours: string | null;
  pro5xMessagesPerFiveHours: string | null;
  pro20xMessagesPerFiveHours: string | null;
  baselineInputCostUsd: number;
  cutdexInputCostUsd: number;
  savedInputCostUsd: number;
  inputCostReductionPercent: number;
  baselinePlanInputCredits: number | null;
  cutdexPlanInputCredits: number | null;
  savedPlanInputCredits: number | null;
};
...
const fallback: Bench = {
  ...
  officialPricing: {
    ...
    subscriptionSourceUrl: "https://learn.chatgpt.com/docs/pricing",
    ...
    models: [
      { model: "gpt-6-astra", label: "GPT-6 Astra", inputPerMillion: 10, cachedInputPerMillion: 1, outputPerMillion: 50, planInputCreditsPerMillion: 250, plusMessagesPerFiveHours: "5–45", pro5xMessagesPerFiveHours: "25–225", pro20xMessagesPerFiveHours: "100–900", baselineInputCostUsd: 53.83, cutdexInputCostUsd: 9.84, savedInputCostUsd: 43.99, inputCostReductionPercent: 81.7, baselinePlanInputCredits: 1345.83, cutdexPlanInputCredits: 246.05, savedPlanInputCredits: 1099.79 },
      { model: "gpt-6-sol", label: "GPT-6 Sol", inputPerMillion: 2, cachedInputPerMillion: 0.2, outputPerMillion: 10, planInputCreditsPerMillion: 50, plusMessagesPerFiveHours: "15–150", pro5xMessagesPerFiveHours: "70–700", pro20xMessagesPerFiveHours: "300–3,000", baselineInputCostUsd: 10.77, cutdexInputCostUsd: 1.97, savedInputCostUsd: 8.80, inputCostReductionPercent: 81.7, baselinePlanInputCredits: 269.17, cutdexPlanInputCredits: 49.21, savedPlanInputCredits: 219.96 },
      { model: "gpt-6-luna", label: "GPT-6 Luna", inputPerMillion: 0.1, cachedInputPerMillion: 0.01, outputPerMillion: 0.5, planInputCreditsPerMillion: 2.5, plusMessagesPerFiveHours: "350–3,000", pro5xMessagesPerFiveHours: "1,750–14,000", pro20xMessagesPerFiveHours: "7,000–56,000", baselineInputCostUsd: 0.54, cutdexInputCostUsd: 0.10, savedInputCostUsd: 0.44, inputCostReductionPercent: 81.7, baselinePlanInputCredits: 13.46, cutdexPlanInputCredits: 2.46, savedPlanInputCredits: 11.00 },
      { model: "gpt-5.6-sol", label: "GPT-5.6 Sol", inputPerMillion: 4, cachedInputPerMillion: 0.4, outputPerMillion: 20, planInputCreditsPerMillion: 100, plusMessagesPerFiveHours: "10–100", pro5xMessagesPerFiveHours: "50–500", pro20xMessagesPerFiveHours: "200–2,000", baselineInputCostUsd: 21.53, cutdexInputCostUsd: 3.94, savedInputCostUsd: 17.60, inputCostReductionPercent: 81.7, baselinePlanInputCredits: 538.33, cutdexPlanInputCredits: 98.42, savedPlanInputCredits: 439.91 },
      { model: "gpt-5.3-codex", label: "GPT-5.3 Codex", inputPerMillion: 3.5, cachedInputPerMillion: 0.35, outputPerMillion: 28, planInputCreditsPerMillion: null, plusMessagesPerFiveHours: null, pro5xMessagesPerFiveHours: null, pro20xMessagesPerFiveHours: null, baselineInputCostUsd: 18.84, cutdexInputCostUsd: 3.44, savedInputCostUsd: 15.40, inputCostReductionPercent: 81.7, baselinePlanInputCredits: null, cutdexPlanInputCredits: null, savedPlanInputCredits: null },
    ],
  },
};
...
export default function App() {
  ...
  return <main>
    ...
    <ModelPricingTable bench={bench} />

    <SavingsCalculator bench={bench} />
...
function Header() { return <header className="wrap site-header"><a className="brand" href="/" aria-label="Cutdex home">cutdex<span>_</span></a><nav><a href="/#usage">usage</a><a href="/#comparison">compare</a><a href="/#estimate">estimate</a><a href="/#connect">connect</a><a href="/#playground">playground</a><a href="/benchmarks">benchmark</a><a href="https://github.com/TaxCollector23/siftline" rel="noreferrer">github ↗</a></nav></header>; }
...
function ModelPricingTable({ bench }: { bench: Bench }) {
  return <section className="model-pricing wrap" id="model-pricing"><div className="model-pricing-head"><div><p className="eyebrow">Official API + plan sensitivity</p><h2>Same measured reduction. Different billing surfaces.</h2></div><p>API dollars and subscription credits are shown separately. Both are input-only scenarios at official standard rates; neither predicts a Codex allowance percentage or a provider invoice.</p></div><div className="comparison-scroll"><table className="comparison-table pricing-table"><thead><tr><th scope="col">Model</th><th scope="col">API input / 1M</th><th scope="col">API before</th><th scope="col">API with Cutdex</th><th scope="col">API saved</th><th scope="col">Plan credits / 1M</th><th scope="col">Plan credits saved</th></tr></thead><tbody>{bench.officialPricing.models.map((model) => <tr key={model.model}><th scope="row">{model.label}</th><td>${model.inputPerMillion.toLocaleString()}</td><td>${model.baselineInputCostUsd.toFixed(2)}</td><td>${model.cutdexInputCostUsd.toFixed(2)}</td><td className="pricing-saved">${model.savedInputCostUsd.toFixed(2)} <small>({model.inputCostReductionPercent.toFixed(1)}%)</small></td><td>{model.planInputCreditsPerMillion === null ? "Not published" : model.planInputCreditsPerMillion.toLocaleString()}</td><td className="pricing-saved">{model.savedPlanInputCredits === null ? "Not published" : `${model.savedPlanInputCredits.toLocaleString()} credits`} {model.savedPlanInputCredits === null ? null : <small>input-only</small>}</td></tr>)}</tbody></table></div><div className="plan-ranges"><span>Published Plus estimate / 5h</span>{bench.officialPricing.models.filter((model) => model.plusMessagesPerFiveHours !== null).map((model) => <span key={model.model}><b>{model.label.replace("GPT-", "")}</b> {model.plusMessagesPerFiveHours} messages</span>)}</div><p className="comparison-note">Verified {bench.officialPricing.verifiedOn}. <a href={bench.officialPricing.sourceUrl} rel="noreferrer">API pricing ↗</a> · <a href={bench.officialPricing.subscriptionSourceUrl} rel="noreferrer">ChatGPT/Codex pricing ↗</a> · {bench.officialPricing.scope} Published Plus ranges are estimates, not fixed limits; actual usage varies with model, context, reasoning, tools, retrieval, and caching.</p></section>;
}
function SavingsCalculator({ bench }: { bench: Bench }) {
  const [monthlyReads, setMonthlyReads] = useState(1_000);
  const [eligibleShare, setEligibleShare] = useState(60);
  const [averageTokens, setAverageTokens] = useState(10_000);
  const eligibleReads = Math.round(monthlyReads * eligibleShare / 100);
  const baselineTokens = eligibleReads * averageTokens;
  const savedTokens = Math.round(baselineTokens * bench.estimatedContextReductionPercent / 100);
  const modeledApiSavings = savedTokens / 1_000_000 * bench.estimatedInputCostUsd.inputPerMillion;
  return <section className="calculator wrap" id="estimate"><div className="calculator-head"><div><p className="eyebrow">Scenario calculator</p><h2>Put your own workload next to the fixture.</h2></div><p>Adjust only the variables you know. This models eligible API-style input context; it never claims to predict a ChatGPT subscription bill.</p></div><div className="calculator-layout"><div className="calculator-controls"><label><span>Structured reads / month</span><output>{monthlyReads.toLocaleString()}</output><input type="range" min="100" max="100000" step="100" value={monthlyReads} onChange={(event) => setMonthlyReads(parseInt(event.target.value, 10))} /></label><label><span>Share eligible for Cutdex</span><output>{eligibleShare}%</output><input type="range" min="0" max="100" step="5" value={eligibleShare} onChange={(event) => setEligibleShare(parseInt(event.target.value, 10))} /></label><label><span>Average baseline context / read</span><output>{averageTokens.toLocaleString()} tokens</output><input type="range" min="1000" max="100000" step="1000" value={averageTokens} onChange={(event) => setAverageTokens(parseInt(event.target.value, 10))} /></label></div><div className="calculator-result"><span>Modeled monthly saving</span><strong>{savedTokens.toLocaleString()} tokens</strong><small>{eligibleReads.toLocaleString()} eligible reads × {averageTokens.toLocaleString()} tokens × {bench.estimatedContextReductionPercent}% measured reduction</small><div><b>${modeledApiSavings.toFixed(2)}</b><span>illustrative API-style input estimate at ${bench.estimatedInputCostUsd.inputPerMillion}/M</span></div></div></div><p className="calculator-note">Scenario only: actual results depend on query shape, source results, model tokenization, system context, output, caching, and whether Codex chooses the optimizer. Verify real API usage with paired traces; subscription usage remains unmeasured.</p></section>;
}
function Benchmarks({ bench }: { bench: Bench }) { return <main><Header /><section className="bench-page wrap"><p className="eyebrow">Deterministic fixture · generated from the repo</p><h1>Measure the part Cutdex can change.</h1><p className="hero-copy">This deterministic suite measures read-result and request-context reduction across {bench.sampleCount} tasks. It does not claim lower provider billing or more subscription usage.</p><div className="bench-table"><div><span>Tool-result token reduction</span><strong><Number value={bench.toolTokenReductionPercent} /></strong></div><div><span>Approx. tool-context reduction</span><strong><Number value={bench.estimatedContextReductionPercent} /></strong></div><div><span>Task-success delta</span><strong>{bench.taskSuccessDelta.toFixed(1)}pp</strong></div><div><span>Quality gate</span><strong>{bench.taskSuccessDelta >= -1 ? "PASS" : "FAIL"}</strong></div></div><div className="method"><h2>What the number means</h2><div><p>Baseline: {bench.baselineContextTokens.toLocaleString()} measured input-context tokens, about ${bench.estimatedInputCostUsd.baseline.toFixed(2)} at ${bench.estimatedInputCostUsd.inputPerMillion}/M. Cutdex: {bench.cutdexContextTokens.toLocaleString()} tokens, about ${bench.estimatedInputCostUsd.cutdex.toFixed(2)}. Estimated saving: ${bench.estimatedInputCostUsd.saved.toFixed(2)}.</p><p>The context figure excludes system prompts, conversation history, reasoning, generated output, and provider tokenization. Run <code>cutdex analyze traces.jsonl</code> against paired provider traces for actual usage and cost.</p><a className="primary-link" href="https://github.com/TaxCollector23/siftline/blob/main/benchmarks/results/latest.md" rel="noreferrer">Open the generated report <span>↗</span></a></div></div><ModelPricingTable bench={bench} /></section><Footer /></main>; }
