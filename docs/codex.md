...
Do not translate the deterministic fixture's 81.7% context reduction into an 81.7% Codex subscription saving. ChatGPT-authenticated Codex usage is charged against the plan's Work/Codex allowance, which is not an API-token invoice and varies with task size, model, reasoning, and output. Only paired provider usage or account usage data can establish a real-world reduction.
